...
async fn publish(
    ...
    let Some(sym_book) = book.get_mut(symbol) else { return };
    let ts = sym_book.last_event_ts;
    let Some(spot) = sym_book.effective_spot() else { return };
    ...
    let Some(snapshot) =
        pipeline.compose(symbol, &candles, chain.as_ref(), spot, ts, synthetic)
    else {
        return;
    };
